                                ...
                                GeometrySet **r_geometry_set)
{
  ...
  const bool is_sdf_mesh = BKE_sdf_object_is_enabled(ob);
  ...
  if (use_deform) {
    for (; md; md = md->next, md_datamask = md_datamask->next) {
      ...
      if (is_sdf_mesh && (mti->flags & eModifierTypeFlag_AcceptsSDF)) {
        continue;
      }
                                 ...
                                 CustomData_MeshMasks &dataMask)
{
  ...
  if (BKE_sdf_object_is_enabled(obedit)) {
                      ...
                      const CustomData_MeshMasks &dataMask)
{
  ...
  if (const Mesh *mesh_eval = BKE_object_get_evaluated_mesh_unchecked(&ob)) {
    BKE_sdf_mesh_runtime_update(ob, *mesh_eval);
  }
  else {
