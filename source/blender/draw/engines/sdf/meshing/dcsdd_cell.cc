...
Sphere::Sphere(double r, int s, const Eigen::Vector3d &c) : radius(r), sign(s), center(c)
{
  /* Basic sanity checks. */
...
static inline double lerp_t(double fa, double fb, double iso = 0.0)
{
...
static Eigen::Vector3d gradientAtPointUsingTrilinear(const Eigen::Matrix<double, 8, 1> &cornerSDF,
                                                     ...
                                                     const Eigen::Vector3d &p)
{
  ...
  const double df_du = (S100 - S000) * omv * omw + (S110 - S010) * v * omw +
                       (S101 - S001) * omv * w + (S111 - S011) * v * w;

  const double df_dv = (S010 - S000) * omu * omw + (S110 - S100) * u * omw +
                       (S011 - S001) * omu * w + (S111 - S101) * u * w;

  const double df_dw = (S001 - S000) * omu * omv + (S101 - S100) * u * omv +
                       (S011 - S010) * omu * v + (S111 - S110) * u * v;
 ...
 * ============================================================================ */
void Cell::fill_hermite_data(const TrueSdfFunc *true_sdf, const TrueSdfGradFunc *true_sdf_grad)
{
  ...
  for (int i = 0; i < 12; ++i) {
    ...
    const int a = EDGE_PAIRS[i][0];
    const int b = EDGE_PAIRS[i][1];

    const double fa = cornerSDF(a);
    const double fb = cornerSDF(b);
    ...
    const Eigen::Vector3d pa = corners.row(a).transpose();
    const Eigen::Vector3d pb = corners.row(b).transpose();
    ...
    if (!use_true) {
       ...
       * GRID-ONLY PATH
       * ============================ */
      const double t = lerp_t(fa, fb); /* between 0 and 1 */

      const Eigen::Vector3d p = (1.0 - t) * pa + t * pb;
      ...
      const Eigen::Vector3d p = 0.5 * (p0 + p1);
...
/* ============================================================
 * update(): QEF minimization
 * ============================================================ */
void Cell::update(const double regularization_weight)
{
  UNUSED_VARS(regularization_weight);
...
Eigen::Vector3d Cell::solve_quadratic_system(const Eigen::MatrixXd &A,
                                             const Eigen::VectorXd &b,
                                             double svd_threshold)
{
  const Eigen::Matrix3d ATA = A.transpose() * A;
  const Eigen::Vector3d ATb = A.transpose() * b;

  /* Compute SVD of AtA. */
  const Eigen::JacobiSVD<Eigen::Matrix3d> svd(ATA, Eigen::ComputeFullU | Eigen::ComputeFullV);
  ...
  /* Pseudoinverse of ATA: (ATA)^+ = V * diag(S_inv) * U^T. */
  const Eigen::Matrix3d ATA_pinv = V * S_inv.asDiagonal() * U.transpose();
  const Eigen::Vector3d centroid = getCentroid();

  /* "Right-hand side" centered at centroid: c = (ATA)^+ (ATb - ATA * centroid). */
  const Eigen::Vector3d rhs = ATb - ATA * centroid;
  const Eigen::Vector3d c = ATA_pinv * rhs;
...
void Cell::minimize_qef(
    double mu, double dc_weight, double sphere_weight, double svd_threshold, bool verbose)
{
  UNUSED_VARS(verbose);
  ...
  const int num_normals = int(normals.size());
  const int num_spheres = int(closest_points_info.size());
  const int num_eqs_regularization = 3;
  ...
  for (int i = 0; i < num_normals; ++i) {
    const Eigen::Vector3d scaled_normal = dc_weight * normals[i];
  ...
  for (const auto &info : closest_points_info) {
    ...
    const Eigen::Vector3d t = alpha * prev_vertex + beta * info.p + gamma * info.fip;
    const Eigen::Vector3d c_to_t_mesh = t - info.c;
    const double rho = c_to_t_mesh.norm();

    /* radius is the distance between q and c. */
    const double radius = (info.q - info.c).norm();

    if (rho > radius) {
      /* Get the index of the sphere in cell.assigned_spheres. */
      const int sphere_idx = info.sphere_idx;
      /* Check the sign of the sphere. */
      const int sign = assigned_spheres[sphere_idx].sign;
      if (sign == -1) {
        continue;
      }
    }
    ...
    const Eigen::Vector3d q = q_sphere;
    const Eigen::Vector3d d = q - info.c;
    const Eigen::Vector3d p = info.p;
    const Eigen::Vector3d fip = info.fip;

    A.row(current_row) = alpha * sqrt_sphere_weight * d.transpose();
    const double b_row_not_scaled = q.dot(d) - beta * p.dot(d) - gamma * fip.dot(d);
  ...
  const Eigen::VectorXd residual = A * vertex - b;
