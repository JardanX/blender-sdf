...
{
  for (int i = 0; i < 12; ++i) {
    /* EDGE_PAIRS is defined in dcsdd_cell.hh. */
    const int p1 = EDGE_PAIRS[i][0];
    const int p2 = EDGE_PAIRS[i][1];
                                           ...
                                           const Eigen::Vector3d &V2)
{
  ...
  const Eigen::Vector3d V0V1 = V1 - V0;
  const Eigen::Vector3d V0V2 = V2 - V0;
  const Eigen::Vector3d V0P = P - V0;

  const double d11 = V0V1.dot(V0V1);
  const double d12 = V0V1.dot(V0V2);
  const double d22 = V0V2.dot(V0V2);
  const double dP1 = V0P.dot(V0V1);
  const double dP2 = V0P.dot(V0V2);
  ...
  const double beta = (d22 * dP1 - d12 * dP2) / denom;
  const double gamma = (d11 * dP2 - d12 * dP1) / denom;
...
void closest_points_on_mesh(const Eigen::MatrixXd &V_mesh,
                            ...
                            Eigen::MatrixXd &closest_points_out,
                            Eigen::VectorXi &face_indices_out)
{
  /* Initialize outputs. */
  ...
  if (F_mesh.rows() == 0 || V_mesh.rows() == 0 || targets.rows() == 0) {
    return;
  }
  ...
  const TriMeshBVH tree(V_mesh, F_mesh);
  tree.squared_distance(targets, sq_distances, I, C);
...
static std::vector<ClosestPointInfo> compute_sphere_to_mesh_closest_points(
    const Cell &cell, const Eigen::MatrixXd &V_mesh, const Eigen::MatrixXi &F_mesh)
{
  std::vector<ClosestPointInfo> results;

  /* Check if mesh is valid before running query. */
  if (V_mesh.rows() == 0 || F_mesh.rows() == 0) {
    return results;
  }
  ...
  for (size_t i = 0; i < N; ++i) {
    centers.row(i) = cell.assigned_spheres[i].center.transpose();
  }

  /* Per-sphere closest-point queries against a locally built tree
   * (the original built a fresh igl::AABB tree here per cell). */
  ...
  const TriMeshBVH tree(V_mesh, F_mesh);
  for (size_t i = 0; i < N; ++i) {
    const Eigen::Vector3d c = centers.row(i).transpose();
    int fid = -1;
    Eigen::Vector3d cp = c;
    tree.squared_distance(c, fid, cp);
    closest_pts.row(i) = cp.transpose();
    face_indices(i) = fid;
  }
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    const Eigen::Vector3d c = sphere.center;
    const double radius = sphere.radius;

    const Eigen::Vector3d t_mesh = closest_pts.row(i).transpose();
    const int tri_idx = face_indices(i);

    /* Calculate the vector from sphere center (c) to the closest mesh point (t_mesh). */
    const Eigen::Vector3d c_to_t_mesh = t_mesh - c;
    ...
    const Eigen::Vector3i tri_indices = F_mesh.row(tri_idx).transpose();

    const Eigen::Vector3d V0 = V_mesh.row(tri_indices(0)).transpose();
    const Eigen::Vector3d V1 = V_mesh.row(tri_indices(1)).transpose();
...
{
  if (!cell.has_vertex || cell.face_intersections.empty()) {
    return;
  }

  /* -----------------------------------------------------
   * Construct local triangle mesh
   * ----------------------------------------------------- */
  ...
  for (const auto &face_data : cell.face_intersections) {
    ...
    for (int edge_i = 0; edge_i < 4; ++edge_i) {
      ...
      const int edge_idx = get_edge_index(corner_pairs[edge_i][0], corner_pairs[edge_i][1]);
  ...
  /* Convert local vertices and faces to Eigen matrices for the closest-point queries. */
  ...
  for (size_t l = 0; l < local_faces.size(); ++l) {
    F_mesh.row(l) = local_faces[l].transpose();
  }
