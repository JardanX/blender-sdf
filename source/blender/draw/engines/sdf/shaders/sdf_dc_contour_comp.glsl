...
float sample_grid(float3 p)
{
  p = clamp(p, float3(0.0), float3(float(grid_verts - 1)));
  int3 i = clamp(int3(floor(p)), int3(0), int3(grid_verts - 2));
  float3 f = p - float3(i);
...
float3 grid_normal(float3 p)
{
  float eps = 0.5;
  return float3(sample_grid(p + float3(eps, 0, 0)) - sample_grid(p - float3(eps, 0, 0)),
              sample_grid(p + float3(0, eps, 0)) - sample_grid(p - float3(0, eps, 0)),
              sample_grid(p + float3(0, 0, eps)) - sample_grid(p - float3(0, 0, eps)));
}

/* Eigenvalues of a 3x3 symmetric matrix (sorted e1 >= e2 >= e3) */
float3 sym3_eigenvalues(float3x3 A)
{
  float p1 = A[0][1] * A[0][1] + A[0][2] * A[0][2] + A[1][2] * A[1][2];
  float q = (A[0][0] + A[1][1] + A[2][2]) / 3.0;

  if (p1 < 1e-30) {
    float3 e = float3(A[0][0], A[1][1], A[2][2]);
...
void main()
{
  ...
  float3 o = float3(grid_origin) + float3(cid) * cell_size;
  float cs = cell_size;

  float3x3 ata = float3x3(0.0);
  float3 atb = float3(0.0);
  float3 mp = float3(0.0);
  float3 an = float3(0.0);
  int ec = 0;

#define EDGE(ox1, oy1, oz1, ox2, oy2, oz2, va, vb)                                   \
  if ((va < 0.0) != (vb < 0.0)) {                                                    \
    float t = va / (va - vb);                                                         \
    float3 p = o + mix(float3(float(ox1), float(oy1), float(oz1)),                        \
                     float3(float(ox2), float(oy2), float(oz2)), t) * cs;               \
    float3 pg = float3(cid) + mix(float3(float(ox1), float(oy1), float(oz1)),               \
                               float3(float(ox2), float(oy2), float(oz2)), t);          \
    float3 n = grid_normal(pg);                                                         \
    float len = length(n);                                                            \
    if (len > 1e-8) {                                                                 \
      n /= len;                                                                       \
...
#undef EDGE
  ...
  float3 eig = sym3_eigenvalues(ata);
  ...
  v = clamp(v, o, o + float3(cs));

  an = normalize(an);
  if (any(isnan(an))) { an = float3(0.0, 1.0, 0.0); }
  ...
  dc_vertices[vi * 2] = float4(v, 0.0);
  dc_vertices[vi * 2 + 1] = float4(an, 0.0);
  dc_cell_verts[ci] = vi;
}
