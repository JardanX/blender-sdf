...
| File | Change |
|------|--------|
| `source/blender/draw/engines/sdf/sdf_engine.cc` | Added a conservative per-brick distance hierarchy build pass and hierarchy textures, and integrated them into the SDF bake/march pipeline alongside the existing dense chunk lookup path. The marcher now binds hierarchy levels built from baked cell minima so empty-space skipping can happen before the exact voxel/cubic refinement path. |
| `source/blender/draw/engines/sdf/shaders/sdf_march_frag.glsl` | Reworked occupied-brick traversal to use conservative hierarchy-guided skipping instead of accepting approximate baked-field hits. Rendering still finishes with the original exact voxel DDA plus cubic solve and the same dual-voxel normal path, but can now jump over locally empty baked regions with tighter quality guarantees. |
| `source/blender/draw/engines/sdf/shaders/infos/sdf_shader_infos.hh` | Exposed the hierarchy build compute shader and added the hierarchy atlas sampler bindings needed by the SDF march shader. |
